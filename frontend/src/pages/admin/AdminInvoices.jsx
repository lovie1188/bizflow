...
import { Download, CheckSquare, Clock, AlertTriangle, FilePlus } from 'lucide-react';
...
const AdminInvoices = () => {
  ...
  const fetchInvoices = async () => {
    try {
      const response = await fetchApi('/invoices?limit=100'); // Assuming we want more for admin view
      const enriched = (response?.data || []).map(inv => {
        ...
        return {
          ...inv,
          calculatedDays: diffDays > 0 ? diffDays : Math.abs(diffDays), // Days overdue or days until due
    ...
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(200px,1fr))', gap:'16px', marginBottom:'32px' }}>
        {[
          ...
          { icon:<AlertTriangle size={22} color="#F59E0B"/>,        label:'Overdue',     value:`₹${totalOverdue.toLocaleString('en-IN')}`,   bg:'rgba(245,158,11,0.08)' },
            ...
            {loading ? (
              ...
              return (
                <tr key={inv.id} style={{ borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
                  ...
                  <td style={{ padding:'16px' }}>₹{parseFloat(inv.amount || 0).toLocaleString('en-IN')}</td>
                  <td style={{ padding:'16px', color:'var(--text-muted)' }}>-</td>
                  <td style={{ padding:'16px', fontWeight:700 }}>₹{parseFloat(inv.amount || 0).toLocaleString('en-IN')}</td>
                  ...
                  <td style={{ padding:'16px' }}>
                    <div style={{ display:'flex', justifyContent:'flex-end', gap:'8px' }}>
                      <button 
                        onClick={() => {
                          if (inv.invoice_url) {
                            window.open(`http://localhost:5000${inv.invoice_url}`, '_blank');
                          } else {
                            alert('Invoice PDF not yet generated or not found.');
                          }
                        }}
                        title="Download PDF" 
